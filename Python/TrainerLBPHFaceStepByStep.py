#   ------------ TESTER FOR THE LBPH FACE RECOGNISER ----------
...
cv2.waitKey(0)
radTrain = open("LBPH/LBPH_PIXEL_RADIUS.txt", "w+")  # open the file to write data
neiTrain = open("LBPH/LBPH_NEIGHBOURS.txt", "w+")   # open the file to write data
cellTrain = open("LBPH/LBPH_CELLS.txt", "w+")       # open the file to write data
...
for (x, y, w, h) in faces:
    ...
    radTrain.write('FACE NUMBER ' + str(faceNum) + '\n')
    for _ in range(54):
    ...
    neighbour = 1
    neiTrain.write('FACE NUMBER ' + str(faceNum) + '\n')
    ...
    cellVal = 1
    cellTrain.write('FACE NUMBER ' + str(faceNum) + '\n')
    for _ in range(50):
